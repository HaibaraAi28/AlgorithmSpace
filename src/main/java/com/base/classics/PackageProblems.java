...
public class PackageProblems {
     ...
     *
     * 利用动态规划求最优值的方法。
     * 假设用dp[N][V]来存储中间状态值,dp[i][j]表示前i件物品能装入容量为j的背包中的物品价值总和的最大值(注意是最大值),
     ...
     * 随之而来的问题是我们要判断第i件物品装到容量为j的背包后,背包内的总价值是否是最大？
     * 如果装了第i件物品后的总价值dp[i-1][j-weight[i]]+value[i]>没装之前的总价值最大值dp[i-1][j]，则肯是最大的；
     ...
     * b)求出背包中装入物品的编号
     *
     * 这里我们采用逆推的思路来处理，如果对于dp[i][j]>dp[i-1][j]，则说明第i个物品肯定被放入了背包，
     * 此时我们再考察dp[i-1][j-weight[i]]的编号就可以了。
     */
    /**
     * 0-1背包问题
     * @param V 背包容量
